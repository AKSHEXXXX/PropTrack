...
export default function EditPropertyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: property, isLoading: fetchLoading } = useSWR<Property>(
  ...
  useEffect(() => {
    if (property) {
      setFormData({
        ...
        property_type: property.property_type || PropertyType.APARTMENT,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({ message: 'Failed to update property' }))
