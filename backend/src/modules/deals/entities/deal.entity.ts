...
export enum DealStatus { PENDING = 'pending', ACTIVE = 'active', CLOSED = 'closed', CANCELLED = 'cancelled' }
...
export class Deal {
  ...
  @Column({ type: 'decimal', precision: 15, scale: 2, nullable: true }) commission_amount: number;
  ...
  @ManyToOne(() => Lead, (l) => l.deals) @JoinColumn({ name: 'lead_id' }) lead: Lead;
  @ManyToOne(() => Agent, (a) => a.deals) @JoinColumn({ name: 'agent_id' }) agent: Agent;
