...
export class DealsController {
  ...
  @Post()
  @ApiOperation({
    summary: 'Create deal from lead (commission auto-calculated)',
  })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get deal with contracts and payments' })
  findOne(@Param('id', ParseIntPipe) id: number) {
    return this.dealsService.findOne(id);
  }

  @Get(':id/audit-log')
  @ApiOperation({ summary: 'Get deal audit log (TRG-03)' })
  getAuditLog(@Param('id', ParseIntPipe) id: number) {
    return this.dealsService.getAuditLog(id);
  }

  @Patch(':id')
  @ApiOperation({
    summary: 'Update deal status (closing triggers property→sold)',
  })
