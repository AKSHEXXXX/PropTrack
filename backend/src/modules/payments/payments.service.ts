...
export class PaymentsService {
  ...
  async create(dto: CreatePaymentDto) {
    const payment = this.paymentRepo.create({
      deal_id: dto.dealId, amount: dto.amount, payment_type: dto.paymentType,
      status: dto.status, reference_no: dto.referenceNo,
  ...
  async findAll() {
    const items = await this.paymentRepo.find({ relations: ['deal'], order: { created_at: 'DESC' } });
    return { data: items, message: 'Payments fetched successfully' };
  }

  async findOne(id: number) {
    const p = await this.paymentRepo.findOne({ where: { payment_id: id }, relations: ['deal'] });
  ...
  async findByDeal(dealId: number) {
    const items = await this.paymentRepo.find({ where: { deal_id: dealId }, order: { payment_date: 'ASC' } });
    const totalPaid = items.filter(p => p.status === 'completed').reduce((sum, p) => sum + Number(p.amount), 0);
    return { data: { items, totalPaid }, message: 'Deal payments fetched successfully' };
  }
