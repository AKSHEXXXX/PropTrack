...
export class LeadsService {
  ...
  async create(dto: CreateLeadDto) {
    ...
    if (existing && !['deal_closed', 'lost'].includes(existing.status)) {
      throw new ConflictException('Active lead already exists for this client and property');
    }
    const lead = this.leadRepo.create({
      client_id: dto.clientId, property_id: dto.propertyId,
      agent_id: dto.agentId, budget: dto.budget, notes: dto.notes,
    });
  ...
  async findAll(filter: LeadFilterDto) {
    ...
    if (filter.agentId) qb.andWhere('lead.agent_id = :agentId', { agentId: parseInt(filter.agentId, 10) });
    if (filter.propertyId) qb.andWhere('lead.property_id = :propertyId', { propertyId: parseInt(filter.propertyId, 10) });
    if (filter.isStale === 'true') qb.andWhere('lead.is_stale = true');
    const [items, total] = await qb.skip((page - 1) * limit).take(limit)
      .orderBy('lead.created_at', 'DESC').getManyAndCount();
    return { data: { items, total, page, limit, totalPages: Math.ceil(total / limit) }, message: 'Leads fetched successfully' };
  }
  ...
  async addTag(leadId: number, tagId: number) {
    const lead = await this.leadRepo.findOne({ where: { lead_id: leadId }, relations: ['tags'] });
  ...
  async autoAssign(leadId: number, agencyId: number) {
    ...
    await this.dataSource.query(`CALL sp_auto_assign_lead($1, $2)`, [leadId, agencyId]);
    const lead = await this.leadRepo.findOne({ where: { lead_id: leadId }, relations: ['agent'] });
