...
export class AppointmentsService {
  ...
  async create(dto: CreateAppointmentDto) {
    ...
    const clash = await this.apptRepo.findOne({
      where: { agent_id: dto.agentId, scheduled_at: new Date(dto.scheduledAt), status: 'scheduled' as any },
    });
    if (clash) throw new ConflictException('Agent already has an appointment at this time');

    const appt = this.apptRepo.create({
      lead_id: dto.leadId, agent_id: dto.agentId, client_id: dto.clientId,
      property_id: dto.propertyId, scheduled_at: new Date(dto.scheduledAt),
  ...
  async findAll(filter: AppointmentFilterDto) {
    ...
    if (filter.agentId) qb.andWhere('a.agent_id = :agentId', { agentId: parseInt(filter.agentId, 10) });
    if (filter.status) qb.andWhere('a.status = :status', { status: filter.status });
    if (filter.from) qb.andWhere('a.scheduled_at >= :from', { from: filter.from });
    if (filter.to) qb.andWhere('a.scheduled_at <= :to', { to: filter.to });
    const [items, total] = await qb.skip((page - 1) * limit).take(limit)
      .orderBy('a.scheduled_at', 'ASC').getManyAndCount();
    return { data: { items, total, page, limit, totalPages: Math.ceil(total / limit) }, message: 'Appointments fetched' };
  }

  async findOne(id: number) {
    const a = await this.apptRepo.findOne({ where: { appointment_id: id }, relations: ['agent', 'client', 'property', 'lead'] });
  ...
  async getUpcoming() {
    const items = await this.apptRepo.find({
      where: { status: 'scheduled' as any, scheduled_at: MoreThanOrEqual(new Date()) },
  ...
  async update(id: number, dto: UpdateAppointmentDto) {
    ...
    if (dto.scheduledAt !== undefined) a.scheduled_at = new Date(dto.scheduledAt);
