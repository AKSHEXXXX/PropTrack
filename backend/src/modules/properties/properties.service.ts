...
export class PropertiesService {
  constructor(
    ...
    @InjectRepository(PropertyImage) private readonly imgRepo: Repository<PropertyImage>,
  ) {}

  async create(dto: CreatePropertyDto) {
    const property = this.propRepo.create({
      agent_id: dto.agentId, title: dto.title, description: dto.description,
      location: dto.location, city: dto.city ?? 'Dubai', price: dto.price,
      area_sqft: dto.areaSqft, property_type: dto.propertyType,
      status: dto.status, bedrooms: dto.bedrooms, bathrooms: dto.bathrooms,
    });
  ...
  async findAll(filter: PropertyFilterDto) {
    ...
    const qb = this.propRepo.createQueryBuilder('p')
      .leftJoinAndSelect('p.agent', 'agent')
      .leftJoinAndSelect('p.images', 'images');
    if (filter.propertyType) qb.andWhere('p.property_type = :pt', { pt: filter.propertyType });
    if (filter.status) qb.andWhere('p.status = :st', { st: filter.status });
    if (filter.city) qb.andWhere('p.city ILIKE :city', { city: `%${filter.city}%` });
    if (filter.minPrice !== undefined) qb.andWhere('p.price >= :min', { min: filter.minPrice });
    if (filter.maxPrice !== undefined) qb.andWhere('p.price <= :max', { max: filter.maxPrice });
    if (filter.minBedrooms !== undefined) qb.andWhere('p.bedrooms >= :mb', { mb: filter.minBedrooms });
    const [items, total] = await qb.skip((page - 1) * limit).take(limit)
      .orderBy('p.created_at', 'DESC').getManyAndCount();
    return { data: { items, total, page, limit, totalPages: Math.ceil(total / limit) }, message: 'Properties fetched successfully' };
  }

  async findOne(id: number) {
    const p = await this.propRepo.findOne({ where: { property_id: id }, relations: ['agent', 'images'] });
  ...
  async remove(id: number) {
    const p = await this.propRepo.findOne({ where: { property_id: id }, relations: ['leads'] });
    if (!p) throw new NotFoundException(`Property #${id} not found`);
    const active = p.leads?.filter(l => !['deal_closed', 'lost'].includes(l.status));
    if (active?.length) throw new BadRequestException('Cannot delete property with active leads');
  ...
  async addImage(id: number, dto: AddImageDto) {
    const p = await this.propRepo.findOne({ where: { property_id: id } });
    if (!p) throw new NotFoundException(`Property #${id} not found`);
    if (dto.isPrimary) await this.imgRepo.update({ property_id: id, is_primary: true }, { is_primary: false });
    const img = this.imgRepo.create({ property_id: id, image_url: dto.imageUrl, is_primary: dto.isPrimary ?? false });
  ...
  async removeImage(propertyId: number, imageId: number) {
    const img = await this.imgRepo.findOne({ where: { image_id: imageId, property_id: propertyId } });
